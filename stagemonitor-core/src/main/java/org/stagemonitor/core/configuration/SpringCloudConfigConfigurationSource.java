...
/**
 * A configuration source to pull stagemonitor configuration from a Spring Cloud Config Server. <p>
 * A simple http GET call is made to retrieve a list of key/value pairs by calling the /{application}-{profile}.properties end point.<br>
 * Multiple property sources are already considered on the server side, so a flat key/value list (separated by line breaks) is returned from the server.<p>
 * The config source can handle additions, changes and removals for properties.<br>
 * The configuration map is only updated when it got a valid response from the server (i.e. 200, not exception, valid response body).
 * On failure the cached properties are still available.<br>
 * Note that {stagemonitor.applicationName} is required with a non default value for this to work, as the application name is used to pull the appropriate configuration.
 * <p>
 * Example as returned from the server when requesting *.properties:
...
public class SpringCloudConfigConfigurationSource extends AbstractConfigurationSource
{
	...
	public SpringCloudConfigConfigurationSource(HttpClient httpClient, String configServerAddress, String applicationName, String profile) {
		...
		this.configAddress = String.format("%s/%s-%s.properties", configServerAddress, applicationName, profile);
		reload();
	}
