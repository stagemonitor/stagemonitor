...
public class AbstractMetricsReporterTest {
	...
	protected Histogram histogram(double mean) {
		return histogram(1L, snapshot(mean));
	}

	protected Histogram histogram(long count, Snapshot snapshot) {
	...
	protected Snapshot snapshot(double mean) {
		return snapshot(mean, 2L, 4L, 5.0, 6.0, 7.0, 8.0, 9.0, 10.0, 11.0);
	}

	protected Snapshot snapshot(double mean, long max, long min, double stdDev, double median, double p75, double p95,
	...
	protected Meter meter(long count) {
	...
	protected Timer timer(double mean) {
		return timer(1L, 2.0, 3.0, 4.0, 5.0, snapshot(mean));
	}

	protected Timer timer(long count, double meanRate, double m1Rate, double m5Rate, double m15Rate, Snapshot snapshot) {
	...
	protected <T> SortedTableLogReporterTest.FluentMap<String, T> map(String name, T metric) {
		final SortedTableLogReporterTest.FluentMap<String, T> map = new SortedTableLogReporterTest.FluentMap<String, T>();
		map.put(name, metric);
		return map;
	}

	protected <T> Gauge gauge(T value) {
		final Gauge gauge = mock(Gauge.class);
		when(gauge.getValue()).thenReturn(value);
		return gauge;
	}

	protected Counter counter(long count) {
		final Counter counter = mock(Counter.class);
		when(counter.getCount()).thenReturn(count);
		return counter;
	}

	protected class FluentMap<K, V> extends TreeMap<K, V> {
